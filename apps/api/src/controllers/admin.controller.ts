...
import { calculateProfitStats } from '../lib/profit';
...
export async function getProfitStats(_req: Request, res: Response) {
  const [orders, totalProducts, totalProductsWithCost] = await Promise.all([
    prisma.order.findMany({
      where: { status: { notIn: ['CANCELLED', 'REFUNDED'] } },
      select: {
        status: true, createdAt: true, total: true, shippingCost: true,
        items: { select: {
          productId: true, quantity: true, price: true, costPrice: true, costRecorded: true,

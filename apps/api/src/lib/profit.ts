...
export function calculateProfitStats(orders: ProfitOrder[], now = new Date()) {
  const allTime = empty();
  const last30Days = empty();
  const today = empty();
  const missingCosts = {
    allTime: new Map<string, MissingCostProduct>(),
    last30Days: new Map<string, MissingCostProduct>(),
    today: new Map<string, MissingCostProduct>(),
  };
  const estimatedCosts = {
    allTime: new Map<string, EstimatedCostProduct>(),
    last30Days: new Map<string, EstimatedCostProduct>(),
    today: new Map<string, EstimatedCostProduct>(),
  };
  ...
  for (const order of orders) {
    ...
    const periods: (keyof typeof missingCosts)[] = ['allTime'];
    if (order.createdAt.getTime() >= cutoff) periods.push('last30Days');
    if (date === todayKey) periods.push('today');
    if (order.createdAt.getTime() >= cutoff) buckets.push(last30Days);
    if (date === todayKey) buckets.push(today);
  ...
  const finishPeriod = (totals: Totals, period: keyof typeof missingCosts) => ({
    ...finish(totals),
